// @ts-nocheck — vendored from reactbits.dev (ts-default variant) with a small
// off-screen render-pause added. Upstream's permissive array indexing trips
// this project's strict noUncheckedIndexedAccess; runtime logic is unchanged.
"use client";
...
class App {
  ...
  setVisible(visible: boolean) {
    ...
    // Re-arm the loop when scrolled back on-screen (update() idles it off-screen).
    if (visible && !this.raf) this.update();
  }

  update() {
    // Fully idle the rAF loop while off-screen: no lerp, no reschedule.
    if (!this.isVisible) {
      this.raf = 0;
      return;
    }
  ...
  destroy() {
    ...
    // Free the GL context immediately rather than waiting for GC — keeps the
    // page under the browser's live-context cap when galleries remount.
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      // Pause the render loop while the gallery is off-screen.
      io = new IntersectionObserver(entries => app?.setVisible(entries[0]?.isIntersecting ?? true), {
        threshold: 0
      });
