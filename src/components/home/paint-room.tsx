...
// A demo, so plain colour names only. These chips used to carry codes like
// "HV-1428" — invented, in a real company's format, for shades that company does
// not sell. The real codes are in the catalogue, where they come from the
// companies' own shade cards.
const SHADES: ReadonlyArray<Shade> = [
  { name: "Terracotta", hex: "#b96b48" },
  { name: "Sage", hex: "#7b8a72" },
  { name: "Slate", hex: "#3e4a52" },
  { name: "Saffron", hex: "#c9a17a" },
  { name: "Oxblood", hex: "#7a3a2f" },
  { name: "Bone", hex: "#e9e2d2" },
];
...
export function PaintRoom() {
  ...
  const pick = (next: Shade) => {
    if (next.name === shade.name) return;
    setPrev(shade);
    setShade(next);
  };
      ...
      <div className="hv-room-inner">
          ...
          <div className="hv-room-chips" role="group" aria-label="Pick a wall colour">
            {SHADES.map((s) => (
              <button
                ...
                data-active={s.name === shade.name || undefined}
                style={{ background: s.hex }}
                onClick={() => pick(s)}
                aria-label={`Paint the wall ${s.name}`}
                aria-pressed={s.name === shade.name}
              />
            ))}
          </div>
        ...
        <div className="hv-room-stage reveal d1" aria-hidden>
          <div className="hv-room-scene">
            ...
            <div key={shade.name} className="hv-room-coat" style={{ background: shade.hex }} />
            ...
            <span className="hv-room-tag">{shade.name}</span>
          </div>
        </div>
      </div>
    </section>
