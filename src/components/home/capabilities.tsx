      ...
      <div className="reveal d1">
        {ROWS.map(([n, t, d], i) => (
          <div key={n} className="hv-cap-row" style={{ display: "grid", gridTemplateColumns: "80px 360px 1fr auto", alignItems: "baseline", gap: 40, padding: "32px 0", borderTop: "1px solid var(--rule)", borderBottom: i === ROWS.length - 1 ? "1px solid var(--rule)" : "none" }}>
            <Mono>{n}</Mono>
            <span className="hv-cap-name" style={{ fontFamily: "var(--serif)", fontWeight: 600, fontSize: 36, lineHeight: 1, color: "var(--fg)" }}>{t}</span>
            <span className="hv-cap-desc" style={{ font: "400 18px/1.5 var(--serif)", color: "var(--fg-mute)", maxWidth: "60ch" }}>{d}</span>
            <Mono>→</Mono>
          </div>
        ))}
      </div>
