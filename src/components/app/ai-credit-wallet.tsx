 ...
 * kiosk, expiring, spendable only by a shop on shop things. This is the other kind: bought
 * with money, never expiring, and held by whoever wants the picture. After a shop hands a
 * project to a customer, that is the customer — an account that can hold no points and buy
 * no plan, so without this there is no way for them to pay for the image at all.
 *
...
export function AiCreditWallet({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section className={`hv-aiw${compact ? " is-compact" : ""}`} aria-labelledby="hv-aiw-title">
      ...
      <p className="hv-aiw-lead">
        ...
        your colour boards. Credits never expire and work on any room.
        {wallet.renderCost > 1 ? ` Each image costs ${wallet.renderCost} credits.` : ""}
      </p>
      ...
      <p className="hv-aiw-fine">
        {buying ? "Opening payment…" : "UPI / cards / netbanking · no expiry"}
      </p>
